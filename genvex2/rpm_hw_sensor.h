...
static const char *const TAG = "RPM_HW";
...
const size_t items_size = ((32) * 4) * 2;

QueueHandle_t fQueue1;
QueueHandle_t fQueue2;

class RPMHWSensor : public PollingComponent {
 public:
  // constructor
  RPMHWSensor() : PollingComponent(1) {}

  Sensor *rpm1 = new Sensor();
  Sensor *rpm2 = new Sensor();
  ...
  void update() override {
    ...
    if( done == true) {
      ...
      // *60 to convert from hz to RPM
      rpm1->publish_state(frequency_hz * 60);
      rpm2->publish_state(frequency_hz2 * 60);
    }
  }
};
